...
class DenseNet:
    ...
    def save_model(self, global_step=None):
        ...
        print ("saved model at step %d" %(global_step))
    ...
    def train_all_epochs(self, train_params, epochs, current_step):
        ...
        for epoch in range(pr_epochs,pr_epochs + n_epochs):
            print("\n", '-' * 30, "Train epoch: %d" % epoch, '-' * 30, '\n')
            start_time = time.time()
            if epoch >= reduce_lr_epoch_1 and epoch < reduce_lr_epoch_2:
                learning_rate = 0.01
            elif epoch >= reduce_lr_epoch_2 and epoch < 330:
                learning_rate = 0.001
            elif epoch >= 330 and epoch < 430:
                learning_rate = 0.0001
            elif epoch >= 430:
                learning_rate = 0.00001
            print("Present learning rate, new lr = %f" % learning_rate)
            ...
            if (self.should_save_model and ((epoch==pr_epochs + n_epochs - 1) or (epoch + 1) % 5==0)):
                self.save_model(global_step=(epoch+1)*1562)
    ...
    def train_one_epoch(self, data, batch_size, learning_rate):
        ...
        for i in range(num_examples // batch_size):
            ...
            #sess = tf.Session(config=configration)
            result=self.sess.run(fetches, feed_dict=feed_dict)
            _, loss, accuracy = result
            total_loss.append(loss)
            total_accuracy.append(accuracy)
            if i % 20 == 0: 
                print("step %d, cross_entropy: %f, accuracy: %f" % (i, loss, accuracy))
            if self.should_save_logs:
                self.batches_step += 1
