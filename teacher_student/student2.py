...
def build_student(images,
    ...
    with tf.name_scope('Block_2'):
        with tf.variable_scope("Student_Block_2"):
            ...
            block2_output = composite_function(block2_output, out_features=342, kernel_size=1, is_training=is_training)
    ...
    with tf.name_scope('FC'):
            with tf.variable_scope("Student_Transition_to_classes"):
                ...
                logits = transition_layer_to_classes(to_1024, n_classes, is_training, name='W')

    return block0_output, block1_output, block2_output, logits


def my_gradient(g0, g1, g2, g3):
    ...
    for i in range(0, len(g3)):
        ...
        if val is not None:
            if i < len(g0):
                g3_copy.append((g3[i][0]+g2[i][0]+g1[i][0]+g0[i][0])/4)
            elif i >= len(g0) and i < len(g1):
                g3_copy.append((g3[i][0]+g2[i][0]+g1[i][0])/3)
...
weight_decay = [1,1,1,1]
lr_decay = [140,210,280,350,420,490,560,630]  #the epoch to reduce learning rate
...
if __name__ == '__main__':
    ...
    with tf.name_scope('teacher_output2'):
        teacher_output2 = tf.placeholder(tf.float32, shape=[None, layer_output[1], layer_output[1], 342], name='input_student')
    with tf.name_scope('teacher_logits'):
        teacher_logits = tf.placeholder(tf.float32, shape=[None, n_classes], name='input_student')
    with tf.name_scope('labels'):
        labels = tf.placeholder(tf.float32, shape=[None, n_classes], name='labels')
    with tf.name_scope('is_training'):
        is_training = tf.placeholder(tf.bool, shape=[])

    student_output0, student_output1, student_output2, student_logits = build_student(images=student_input,
    ...
    with tf.name_scope("l2_3"):
        l2_3 = tf.reduce_mean(tf.nn.l2_loss(student_logits-teacher_logits))
    tf.summary.scalar('l2_0', l2_0)
    tf.summary.scalar('l2_1', l2_1)
    tf.summary.scalar('l2_2', l2_2)
    tf.summary.scalar('l2_3', l2_3)

    # choose related variables
    gradient_all = optimizer.compute_gradients(l2_3)
    ...
    with tf.name_scope("l2_loss2"):
        l2_loss2 = tf.add_n([tf.nn.l2_loss(var) for var in init+grads_vars0+grads_vars1+grads_vars2])
    with tf.name_scope("l2_loss3"):
        l2_loss3 = tf.add_n([tf.nn.l2_loss(var) for var in init+grads_vars0+grads_vars1+grads_vars2+trans])
    tf.summary.scalar('l2_loss0', l2_loss0)
    tf.summary.scalar('l2_loss1', l2_loss1)
    tf.summary.scalar('l2_loss2', l2_loss2)
    tf.summary.scalar('l2_loss3', l2_loss3)

    # compute total loss
    with tf.name_scope("loss0"):
        loss0 = l2_0+l2_loss0*weight_decay[0]
    with tf.name_scope("loss1"):
        loss1 = l2_1+l2_loss1*weight_decay[1]
    with tf.name_scope("loss2"):
        loss2 = l2_2+l2_loss2*weight_decay[2]
    with tf.name_scope("loss3"):
        loss3 = l2_3+l2_loss3*weight_decay[3]
    ...
    with tf.name_scope('gradient2'):
        gradient2 = optimizer.compute_gradients(loss2, init+grads_vars0+grads_vars1+grads_vars2)
    with tf.name_scope('gradient3'):
        gradient3 = optimizer.compute_gradients(loss3, init+grads_vars0+grads_vars1+grads_vars2+trans)

    grads_holder = [(tf.placeholder(tf.float32, shape=g.get_shape()), v) for (g, v) in gradient3]
    ...
    #cnt = global_epoch // 30 # automatical learning rate decrease, every 30 epochs
   # lr = float(lr / (2**cnt)) # divide by 2
    ...
    for epoch in range(global_epoch, total_epoch):
        ...
        if epoch in lr_decay:#counter == 30:
            lr = float(lr / 2)
            #counter = 0
        ...
        for i in range(0, train_params['iterations of one epoch']):
            ...
            a, b, c, d, acc, summ, w, x, y, z = sess.run([gradient0, gradient1, gradient2, gradient3, accuracy,
                                                merged, loss0, loss1, loss2, loss3], feed_dict=feed_dict)
        
            avg_gradient = my_gradient(a, b, c, d)
            avg_gradient = np.asarray(avg_gradient)
            grads_sum = {learning_rate: lr}
            for t in range(len(grads_holder)):
                k = grads_holder[t][0]
                if k is not None:
                    grads_sum[k] = avg_gradient[t]
            sess.run(train_op, feed_dict=grads_sum)

            block0_loss.append(w)
            block1_loss.append(x)
            block2_loss.append(y)
            total_loss.append(z)
            accuracy_s.append(acc)
 
            summary = tf.Summary(value=[
                tf.Summary.Value(
                    tag='loss0_per_batch', simple_value=float(w)),
                tf.Summary.Value(
                    tag='loss1_per_batch', simple_value=float(x)),
                tf.Summary.Value(
                    tag='loss2_per_batch', simple_value=float(y)),
                tf.Summary.Value(
            ...
            print 'step--%d: student accuracy: %f; l2_0: %f; l2_1:%f; l2_2:%f; l2_3:%f' % (i, acc, w, x, y, z)
            print '-----------------'
        ...
        mean_loss2 = np.mean(np.asarray(block2_loss))
        ...
        summary = tf.Summary(value=[
            ...
            tf.Summary.Value(
                tag='loss2_train', simple_value=float(mean_loss2)),
            tf.Summary.Value(
