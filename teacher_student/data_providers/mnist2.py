...
def read_mnist(filenames):
    ...
    logits = f['logits'][:]
    f.close()
    return images, labels, block0, block1, block2, logits


class MnistDataSet(ImagesDataSet):
    def __init__(self, images, labels, block0, block1, block2, logits, n_classes, shuffle, normalization,
                 augmentation):
        """
        Args:
    ...
    def next_batch(self, batch_size, is_training):
        ...
        if is_training==True:
            ...
            if (self._train_batch_counter>=60000//batch_size):
                end = -1
...
class MNISTDataProvider(DataProvider):
    ...
    def __init__(self, save_path=None, validation_set=None,
        ...
        images, labels, block0, block1, block2, logits = read_mnist(train_fnames)
    ...
    def save_path(self):
        if self._save_path is None:
            self._save_path = './mnist' #tempfile.gettempdir()
...
class MnistDataProvider(MNISTDataProvider):
    ...
    def get_filenames(self, save_path):
        train_filenames = os.path.join(save_path, 'train_data.h5') #change to your training data file
        test_filenames = os.path.join(save_path, 'test_data.h5')  #change to your testing data file
