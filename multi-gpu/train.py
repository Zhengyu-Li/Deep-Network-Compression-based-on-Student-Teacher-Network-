  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
      ...
      for i in xrange(args.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('Tower_%d' % i) as scope:
            ...
            total_loss = tf.add(cross_entropy_mean, reg_loss*0.0001)
    ...
    for epoch in xrange(start_epoch, start_epoch + args.num_epochs):
      ...
      for step in xrange(args.num_batches):
        ...
        top5.append(float(topn_accuracy))
        cost.append(float(loss_value))
        # Check for errors
        ...
        if step % 10 == 0:
          ...
          sys.stdout.flush()
        if step % 100 == 0:
          mean_top1 = np.mean(np.asarray(top1))
          mean_top5 = np.mean(np.asarray(top5))
          mean_cost = np.mean(np.asarray(cost))
          print ('mean top1: %f, mean top5: %f, mean loss: %f' % (mean_top1,mean_top5,mean_cost))
          ...
          summary = tf.Summary(value=[
            tf.Summary.Value(
                tag='mean_top1', simple_value=float(mean_top1)),
            tf.Summary.Value(
                tag='mean_top5', simple_value=float(mean_top5)),
            tf.Summary.Value(
                tag='mean_loss', simple_value=float(mean_cost)),
            ])
          summary_writer.add_summary(summary, step)
          del top1[:]
          del top5[:]
          del cost[:]
      checkpoint_path = './multi-gpu/dense-190/' #choose your folder to store model
...
def main():  # pylint: disable=unused-argument
    ...
    parser.add_argument('--batch_size', default= 48, type= int, action= 'store', help= 'The training batch size')
    ...
    parser.add_argument('--num_epochs', default= 300, type= int, action= 'store', help= 'The number of epochs')
    parser.add_argument('--data_info', default= './cifar100/train', action= 'store', help= 'Name of the cifar100 file')
    ...
    parser.add_argument('--log_dir', default= './multi-gpu/dense-190', action= 'store', help= 'Path for saving Tensorboard info')
    parser.add_argument('--snapshot_prefix', default= './multi-gpu/dense-190', action= 'store', help= 'Prefix for checkpoint files')
    parser.add_argument('--architecture', default= 'densenet', help= 'The DNN architecture')
    parser.add_argument('--depth', default= 190, type= int, action= 'store', help= 'The depth of ResNet architecture')
    parser.add_argument('--run_name', default= 'Run'+str(time.strftime("-%d-%m-%Y_%H-%M-%S")), action= 'store', help= 'Name of the experiment')
    parser.add_argument('--num_gpus', default= 2, type= int, action= 'store', help= 'Number of GPUs')
    ...
    parser.add_argument('--retrain_from', default= './multi-gpu/dense-190/', action= 'store', help= 'Continue Training from a snapshot file')
    parser.add_argument('--log_debug_info', default= True, action= 'store', help= 'Logging runtime and memory usage info')
    ...
    parser.add_argument('--LR_steps', type=int, nargs='+', default=[136, 175, 210], help='LR change epochs')
    parser.add_argument('--LR_values', type=float, nargs='+', default=[0.05, 0.01, 0.001, 0.0001], help='LR change epochs')
