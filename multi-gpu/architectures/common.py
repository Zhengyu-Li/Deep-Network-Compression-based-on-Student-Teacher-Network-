...
def _get_variable(name,
    ...
    with tf.device('/cpu:0'):
      var = tf.get_variable(name,
                           shape= shape if initializer != 'tf.constant(0.0)' else None,
...
def fullyConnected(x, num_units_out, wd= 0.0, weight_initializer= None, bias_initializer= None):
    ...
    if weight_initializer is None:
      weight_initializer = tf.contrib.layers.xavier_initializer()#tf.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.float32)
    if bias_initializer is None:
      bias_initializer = tf.constant(0.0, shape=[num_units_out])#tf.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.float32) 

    weights = _get_variable('weights',
                            [num_units_in, num_units_out], weight_initializer, tf.contrib.layers.l2_regularizer(wd))

    biases = _get_variable('biases',None, bias_initializer)
...
def spatialConvolution(x, ksize, stride, filters_out, wd= 0.0, weight_initializer= None, bias_initializer= None):
    ...
    if weight_initializer is None:
      weight_initializer = tf.contrib.layers.variance_scaling_initializer()#tf.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.float32)
    if bias_initializer is None:
      bias_initializer = tf.constant(0.0, shape=[filters_out])#tf.random_uniform_initializer(minval= -stddev, maxval= stddev, dtype= tf.float32) 
